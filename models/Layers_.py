...
#         # print shapes if needed
#         # print(f'input: {input.size()}')
#         # print(f'input sliced: {input[None,:,:].size()}')
#         # print(f'gamma: {gamma}')
#         t=torch.abs(input[None,:,:])< abs_z*gamma
#         grad_input=grad_input*torch.mean(t*abs_z, dim=0)/(2*gamma)

#         return grad_input, None    

class LIFSpikeW(nn.Module):
    def __init__(self, thresh=1.0, tau=1., gama=1.0):
    # def __init__(self, thresh=1.0, tau=0.5, gama=0.5):
        super(LIFSpikeW, self).__init__()
        #self.act = ZIF.apply
