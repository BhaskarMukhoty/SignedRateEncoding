...
def main():
    ...
    seed_all(args.seed)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True, drop_last=False)
    ...
    for atk_config in config:
        ...
        #f_name =os.path.join(log_dir, '%s.pickle'%(str(args.identifier)+'_list'))
